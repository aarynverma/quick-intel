...
export default function StyleDropdown({ value, onChange, disabled }: Props) {
  ...
  return (
    <div>
      <label className="mb-1.5 block text-xs font-medium text-slate-700">Summary style</label>
      <div className="relative">
        <select
          value={value}
          onChange={(e) => onChange(e.target.value as SummaryStyle)}
          disabled={disabled}
          className="w-full cursor-pointer appearance-none rounded-lg border border-slate-200 bg-white px-3 py-2 pr-9 text-sm text-slate-900 shadow-sm transition hover:border-slate-300 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-100 disabled:cursor-not-allowed disabled:bg-slate-50"
        >
          {OPTIONS.map((o) => (
