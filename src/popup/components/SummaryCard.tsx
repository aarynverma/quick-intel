...
export default function SummaryCard({ record }: Props) {
  ...
  return (
    <article className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
      ...
      <div className="p-3">
        <h2 className="mb-2 line-clamp-2 text-sm font-semibold text-slate-900 dark:text-slate-100">{record.title}</h2>
        <div className="whitespace-pre-wrap text-[13px] leading-relaxed text-slate-700 dark:text-slate-300">
          {record.content}
        </div>
      </div>
      ...
      <div className="flex gap-2 border-t border-slate-100 bg-slate-50/50 px-3 py-2 dark:border-slate-700 dark:bg-slate-800/50">
        ...
        <button
          onClick={handleOpen}
          className="flex-1 rounded-md border border-slate-200 bg-white px-2.5 py-1.5 text-xs font-medium text-slate-700 transition hover:bg-slate-100 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600"
        >
          Open page
        </button>
      </div>
